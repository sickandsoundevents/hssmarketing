...
const BUILD = "9"; // erhoehen bei jeder Aenderung, sichtbar unter /api/status
...
async function fetchWeeztixOrders() {
  ...
  // Der Endpunkt erwartet eine ElasticSearch Abfrage im Body (POST).
  ...
  // ElasticSearch liefert hits.hits[]._source; sonst data oder ein Array.
...
// Diagnose: ruft den echten Statistik Endpunkt auf und zeigt die rohe Antwort.
app.get("/api/raw", async (req, res) => {
  ...
  try {
    ...
    res.json({ url, status: r.status, sent: body, body: (await r.text()).slice(0, 1800) });
  } catch (e) {
