...
app.get("/api/raw", async (req, res) => {
  ...
  const twoY = iso(new Date(Date.now() - 2 * 365 * 864e5));
  const base = { timeunit: "year", offset: 0, sorting: { created_at: "desc" }, filters: {}, events: [] };
  const variants = {
    exact_team: { ...base, start: "2025-06-12T00:00:00.000Z", end: "2026-06-12T00:00:00.000Z", limit: 10 },
    team_limit100: { ...base, start: "2025-06-12T00:00:00.000Z", end: "2026-06-12T00:00:00.000Z", limit: 100 },
    wide_2y_100: { ...base, start: twoY, end: iso(new Date()), limit: 100 },
    my_orig_1000: { ...base, start: "2024-01-01T00:00:00.000Z", end: iso(new Date()), limit: 1000 },
  };
  const out = {};
  for (const [name, body] of Object.entries(variants)) {
    try {
      ...
      let total = null, keys = null;
      try {
        const j = JSON.parse(text);
        total = j?.hits?.total?.value ?? j?.hits?.total ?? null;
        const src = j?.hits?.hits?.[0]?._source;
        keys = src ? Object.keys(src) : null;
      } catch {}
      out[name] = { status: r.status, total, firstOrderKeys: keys, body: keys ? undefined : text.slice(0, 220) };
    } catch (e) {
