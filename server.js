...
const BUILD = "7"; // erhoehen bei jeder Aenderung, sichtbar unter /api/status
...
// Diagnose: probiert mehrere moegliche Weeztix Adressen durch.
app.get("/api/probe", async (req, res) => {
  ...
  const candidates = [
    ...
    ["GET", `${B}/companies`],
    ["GET", `${B}/company/${id}`],
    ["GET", `${B}/company/${id}/orders`],
    ["POST", `${B}/company/${id}/orders`, { size: 1, query: { match_all: {} } }],
    ["GET", `${B}/3.0.0/company/${id}/orders`],
    ["POST", `${B}/order/advancedSearch`, {}],
  ];
  const out = [];
  for (const [method, url, body] of candidates) {
    try {
      ...
      out.push({ method, url, status: r.status, body: (await r.text()).slice(0, 180) });
    } catch (e) {
