...
const BUILD = "12"; // erhoehen bei jeder Aenderung, sichtbar unter /api/status
...
function weeztixStatsBody() {
  const fmt = (d) => d.toISOString().replace(/\.\d{3}Z$/, "+00:00");
  return {
    offset: 0,
    limit: 1000,
    timeunit: "month",
    range_applies_to: "created_at",
    statii: ["paid"],
...
async function fetchWeeztixOrders() {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify(weeztixStatsBody()),
  });
  if (!res.ok) {
    const body = (await res.text()).slice(0, 300);
    throw new Error(`Weeztix Orders HTTP ${res.status} (POST ${url}) ${body}`);
  }
  const json = await res.json();
  // ElasticSearch Standardform: hits.hits[]._source
...
// Diagnose v2: setzt den Company Header und testet advancedSearch method Werte.
app.get("/api/probe", async (req, res) => {
  let token;
  try { token = await weeztixAccessToken(); }
  catch (e) { return res.json({ error: "kein Weeztix Token: " + e.message }); }
  const id = CFG.weeztixCompanyId;
  const B = WEEZTIX_API;
  const H = { Authorization: `Bearer ${token}`, Company: id };
  const Hj = { ...H, "Content-Type": "application/json" };
  const candidates = [
    ["GET", "https://auth.weeztix.com/users/me", null, H],
    ["GET", `${B}/order`, null, H],
    ["GET", `${B}/company/${id}/orders`, null, H],
    ["POST", `${B}/order/advancedSearch`, { method: "all" }, Hj],
    ["POST", `${B}/order/advancedSearch`, { method: "company" }, Hj],
    ["POST", `${B}/order/advancedSearch`, { method: "elasticsearch", company: id, size: 1 }, Hj],
  ];
  const out = [];
  for (const [method, url, body, headers] of candidates) {
    try {
      const r = await fetch(url, { method, headers, ...(body ? { body: JSON.stringify(body) } : {}) });
      out.push({ method, url, sent: body || null, status: r.status, body: (await r.text()).slice(0, 220) });
    } catch (e) {
      out.push({ method, url, error: e.message });
    }
  }
  res.json(out);
});

// Diagnose: testet mehrere Bodys, um die funktionierende Variante zu finden.
app.get("/api/raw", async (req, res) => {
  ...
  const recent = fmt(new Date(Date.now() - 30 * 864e5));
  const variants = {
    A_minimal: { offset: 0, limit: 10, timeunit: "year" },
    B_range: { offset: 0, limit: 10, timeunit: "day", range_applies_to: "created_at", start: recent, end: now },
    C_paid: { offset: 0, limit: 10, timeunit: "day", range_applies_to: "created_at", statii: ["paid"], start: recent, end: now },
    D_full: { offset: 0, limit: 0, timeunit: "year", range_applies_to: "created_at", statii: ["paid"], start: recent, end: now, purchase_channels: ["shop"] },
    E_nodates: { offset: 0, limit: 10, timeunit: "month", range_applies_to: "created_at", statii: ["paid"] },
  };
  const out = {};
