// =============================================================================
// HSS Marketing Attribution  -  Server (Statistik Abruf, finale Zuordnung)
// =============================================================================
...
// Zuordnung event_id -> Showname. Weitere Shows hier ergaenzen.
const EVENT_NAMES = {
  "f95bbef5-662b-446b-9c19-c6a5549be66b": "Maimarkthalle Mannheim",
};

function weeztixConnected() { return !!(fs.existsSync(TOKEN_FILE) && CFG.weeztixClientId && CFG.weeztixCompanyId); }
function isLive() { return !!(weeztixConnected() && CFG.metaToken && CFG.metaAdAccount); }
...
async function weeztixAccessToken() {
  ...
  const res = await fetch(WEEZTIX_TOKEN, {
    ...
    body: new URLSearchParams({ grant_type: "refresh_token", refresh_token: t.refresh_token, client_id: CFG.weeztixClientId, client_secret: CFG.weeztixClientSecret }),
  });
...
// --- Bestellungen seitenweise holen ------------------------------------------
async function fetchAllOrders() {
  const token = await weeztixAccessToken();
  const url = `${WEEZTIX_API}/statistics/orders/${CFG.weeztixCompanyId}`;
  const limit = 100, maxPages = 60;
  let all = [], offset = 0;
  for (let p = 0; p < maxPages; p++) {
    const body = {
      start: "2024-01-01T00:00:00.000Z", end: new Date().toISOString(),
      timeunit: "year", offset, limit, sorting: { created_at: "desc" }, filters: {}, events: [],
    };
    ...
    if (!res.ok) throw new Error(`Weeztix HTTP ${res.status} ${(await res.text()).slice(0, 150)}`);
    const json = await res.json();
    const hits = json?.hits?.hits ? json.hits.hits.map((h) => h._source || h) : (Array.isArray(json?.data) ? json.data : []);
    all = all.concat(hits);
...
// --- Felder aus einer Bestellung lesen ---------------------------------------
function pickTech(o) {
  const t = o?.tech_data || {};
  let dec = t.decoded;
  if (!dec && typeof t.data === "string") { try { dec = JSON.parse(t.data); } catch { dec = {}; } }
  return { t, dec: dec || {} };
}
function hostOf(u) { try { return new URL(u).hostname; } catch { return null; } }

function extractOrderFields(o) {
  const evId = o?.tickets?.[0]?.event_id || "unbekannt";
  const show = EVENT_NAMES[evId] || ("Event " + String(evId).slice(0, 8));
  const tickets = Array.isArray(o?.tickets) ? o.tickets.length : 1;
  const revenue = Number(o?.finn_value || o?.finn_price || 0) / 100; // Cent -> Euro
  const { t, dec } = pickTech(o);
  let channel = t.utm_campaign || t.utm_source || t.click_id_source || null;
  if (!channel && dec.referrer) channel = hostOf(dec.referrer);
  if (!channel) channel = "Direkt";
...
async function fetchMetaSpend() {
  ...
  const url = `https://graph.facebook.com/v20.0/${acct}/insights?level=campaign&fields=campaign_name,spend&date_preset=maximum&limit=500&access_token=${encodeURIComponent(CFG.metaToken)}`;
  const res = await fetch(url);
...
function buildLiveAttribution(orders, spendByChannel) {
  const map = {};
  orders.forEach((o) => {
    const { show, tickets, revenue, channel } = extractOrderFields(o);
    const key = show + "||" + channel;
    if (!map[key]) map[key] = { show, channel, orders: 0, revenue: 0, spend: 0 };
    map[key].orders += tickets;     // Einheit: verkaufte Tickets
...
// --- Zwischenspeicher ---------------------------------------------------------
let CACHE = { rows: null, builtAt: 0, ordersCount: 0, error: null };
let refreshing = false;
async function refreshCache() {
  if (refreshing) return; refreshing = true;
  try {
    const orders = await fetchAllOrders();
    let spend = {}; try { spend = await fetchMetaSpend(); } catch (e) { /* Meta optional */ }
    CACHE = { rows: buildLiveAttribution(orders, spend), builtAt: Date.now(), ordersCount: orders.length, error: null };
...
app.get("/connect", (req, res) => {
  ...
  const params = new URLSearchParams({ client_id: CFG.weeztixClientId, redirect_uri: `${CFG.publicUrl}/oauth/callback`, response_type: "code", state: "hss" });
...
app.get("/oauth/callback", async (req, res) => {
  try {
    const tokRes = await fetch(WEEZTIX_TOKEN, {
      method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ grant_type: "authorization_code", code: req.query.code, client_id: CFG.weeztixClientId, client_secret: CFG.weeztixClientSecret, redirect_uri: `${CFG.publicUrl}/oauth/callback` }),
    });
...
app.get("/api/data", async (req, res) => {
  ...
  const stale = Date.now() - CACHE.builtAt > 5 * 60 * 1000;
  if (!CACHE.rows) await refreshCache();
  else if (stale) refreshCache();
  if (!CACHE.rows) return res.json({ mode: "demo", error: CACHE.error, rows: DEMO_ROWS });
  res.json({ mode: "live", ordersCount: CACHE.ordersCount, builtAt: CACHE.builtAt, error: CACHE.error, rows: CACHE.rows });
});

// Schneller Mapping Check: erste fuenf Bestellungen ausgelesen.
app.get("/api/raw", async (req, res) => {
  try {
    const token = await weeztixAccessToken();
    const r = await fetch(`${WEEZTIX_API}/statistics/orders/${CFG.weeztixCompanyId}`, {
      method: "POST", headers: { Authorization: `Bearer ${token}`, Company: CFG.weeztixCompanyId, "Content-Type": "application/json" },
      body: JSON.stringify({ start: "2024-01-01T00:00:00.000Z", end: new Date().toISOString(), timeunit: "year", offset: 0, limit: 5, sorting: { created_at: "desc" }, filters: {}, events: [] }),
    });
    const j = await r.json();
    const hits = (j?.hits?.hits || []).map((h) => h._source || h);
    res.json({ total: j?.hits?.total?.value ?? null, mapped: hits.map(extractOrderFields) });
  } catch (e) { res.json({ error: e.message }); }
});

app.get("/api/refresh", async (req, res) => { await refreshCache(); res.json({ ok: true, ordersCount: CACHE.ordersCount, error: CACHE.error }); });

app.get("/api/status", (req, res) => {
  res.json({
    build: BUILD, weeztixConnected: weeztixConnected(), metaConfigured: !!(CFG.metaToken && CFG.metaAdAccount), live: isLive(),
    cache: { built: CACHE.builtAt ? new Date(CACHE.builtAt).toISOString() : null, ordersCount: CACHE.ordersCount, error: CACHE.error },
  });
});

app.use(express.static(path.join(__dirname, "public")));
app.listen(PORT, () => { console.log(`HSS Marketing build ${BUILD} auf Port ${PORT}`); if (isLive()) setTimeout(refreshCache, 1500); });
